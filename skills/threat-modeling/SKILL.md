...
the residual risk. This skill is the deep, repeatable method — pick the technique that
fits the question.

## Pick the method

- **STRIDE** — per-element decomposition. Best default for "what can go wrong at this
...
- **PASTA** — risk-centric, seven stages from business objectives to attack simulation.
  Use when threats must be prioritized by business impact, not just enumerated.
- **Attack trees** — goal-oriented. Root is the attacker's objective; branches are the
  paths. Use to reason about a specific high-value target end-to-end.
- **DFD-driven** — draw the data-flow diagram first, then apply STRIDE to each element
  and flow. Use when the system is large enough that "which components" isn't obvious.
